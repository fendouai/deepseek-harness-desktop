...
Users can run a rigged VRM character with text-synchronous mouth expressions, facial states, blinking, and procedural skeletal movement, switch among four bundled image characters, or change the flat Avatar with one image without expanding desktop authority. The Avatar remains inside the main application window, activity is coarse, and there is no synthesized voice, audio-timed lip sync, dragging, user VRM import, or native always-on-top placement. Those additions require independent speech and asset contracts rather than growth inside this presentation plugin.
