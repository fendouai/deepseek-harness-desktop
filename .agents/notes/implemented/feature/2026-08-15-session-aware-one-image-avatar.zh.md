...
同一个投影负责 Avatar 对话投递：`Talk` 通过当前 Session 的常规排队 `prompt()` 路径发送用户确认过的文本，因此 Avatar 文字和麦克风转录与主输入框具有相同的接纳、记录与任务语义。Web Speech Recognition 提供可选转录，用户启用回复朗读后，Speech Synthesis 朗读新完成的 Assistant 文本。不支持语音识别时只在本地失败，不削弱文字对话。打包后的 macOS 应用声明麦克风、语音识别与 hardened runtime 音频输入权限。调试运行时 URL 带有显式标记，防止裸 `tauri dev` 二进制在没有应用 bundle 时调用隐私敏感的语音 API。系统语音事件还会在朗读回复期间维持口型动画。
...
用户可以运行具备明显待机动作、文本同步口型、面部状态、眨眼和程序化骨骼动作的已绑定 VRM 角色；通过文字或平台支持的系统语音与当前任务对话；收听已完成回复；切换四个内置图片角色；或用一张图片更换平面 Avatar。Avatar 仍位于主应用窗口内，活动状态较粗，而且没有 provider 提供的音频时间轨、拖动、用户 VRM 导入或原生置顶展示。这些能力需要独立的语音与资源契约，而不是继续扩张这个展示插件。
