...
同一个投影负责 Avatar 对话投递：`Talk` 通过当前 Session 的常规排队 `prompt()` 路径发送用户确认过的文本，因此 Avatar 文字和麦克风转录与主输入框具有相同的接纳、记录与任务语义。Web Speech Recognition 提供可选转录，用户启用回复朗读后，Speech Synthesis 朗读新完成的 Assistant 文本。不支持语音识别时只在本地失败，不削弱文字对话。macOS bundle 声明麦克风、语音识别与 hardened runtime 音频输入权限。系统语音事件还会在朗读回复期间维持口型动画。
