...
同一个投影负责 Avatar 对话投递：`Talk` 通过当前 Session 的常规排队 `prompt()` 路径发送文字和最终麦克风转录，因此两类输入与主输入框具有相同的接纳、记录与任务语义。单次 `Speak` 请求在识别完成后自动发送。可选免手持模式采用防回声的轮流说话：监听、发送前停止、等待不同序号的 Assistant 最终回复、播放已启用的语音输出，然后恢复监听；扬声器播放期间不会保持麦克风开启。不支持语音识别时只在本地失败，不削弱文字对话。打包后的 macOS 应用声明麦克风、语音识别与 hardened runtime 音频输入权限。调试运行时 URL 带有显式标记，防止裸 `tauri dev` 二进制在没有应用 bundle 时调用隐私敏感的语音 API。系统语音事件还会在朗读回复期间维持口型动画。
