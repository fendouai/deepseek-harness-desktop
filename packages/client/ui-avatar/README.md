...
The `Talk` control sends typed text directly to the currently selected Harness task through its ordinary queued-prompt method. Its `Speak` control uses the Web Speech Recognition API when the platform WebView exposes it and automatically sends the finalized transcript after the user pauses. Optional hands-free conversation alternates listening, task execution, and reply playback; listening resumes after the new reply finishes, rather than remaining active during speaker output and feeding the reply back into the microphone. `Read Assistant replies aloud` uses the system Speech Synthesis API and drives the same VRM mouth targets while audio is playing. Packaged macOS apps declare microphone, speech-recognition, and audio-input permissions. A bare `tauri dev` binary cannot carry those bundle declarations, so its microphone control stops locally instead of invoking macOS privacy services. A platform without Web Speech Recognition keeps text conversation and reply reading available and reports the unsupported microphone locally.
